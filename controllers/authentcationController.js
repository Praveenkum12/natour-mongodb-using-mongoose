...
exports.restrictTo = function (...roles) {
  return function (req, res, next) {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError(`You didn't have the permission to do this action.`, 403)
      );
    }
...
exports.forgotPassword = catchAsync(async function (req, res, next) {
  ...
  try {
    ...
    await user.save({ validatesBeforeSave: false });
...
exports.resetPassword = catchAsync(async function (req, res, next) {
  ...
  // 2) If the user not expires, and there is user, set the new password
  if (!user) {
...
exports.updatePassword = catchAsync(async function (req, res, next) {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(
      new AppError(
        'The password you enterd is not correct. Please login again...',
        401
      )
    );
  }
